...
void compute_PR(int threadId, int helperTid, multi_atomic_var1 *local_error_) {
  ...
  while(i <= nodes && local_error_->iteration == old_values->iteration) {
    ...
    double pr_buffer = damping_factor/nodes;

    for(int j=IA[i-1];j<IA[i];j++) {
      pr_buffer += (prev_pr[JA[j]].load(std::memory_order_seq_cst)->rank/out_edges[JA[j]]) * (1-damping_factor);
    }
...
int main(int argc, char** argv)
{
  ...
  cout << "Time : " << time << "\n";
  std::ofstream outFile("Barrier_helper_td_fail_out.txt");
  outFile << time << "\n";
  outFile << avg_itr << "\n";
  outFile << sum << "\n";
