...
programs = ["Barriers.cpp", "No_Sync.cpp", "Barrier_helper.cpp"]
...
#		for sz in [100, 150, 200, 250, 300, 350, 400, 450, 500] :
		for sz in [550, 600, 650, 700, 750, 800, 850, 900, 950, 1000] :
			for count in range(0, 1) :
				cmd2 = "./a.out " + input_files[i] + " 32 " + str(start_nodes[i]) + " " + str(max_nodes[i]) + " " + str(iteration_threshold) + " " + str(sz)
				print cmd2
				cmd2 = Popen(cmd2.split(), stdin=sp.PIPE, stdout=sp.PIPE)
				cmd2.wait()

				cmd3 = "mv " + program[:-4] + "_out.txt " + program[:-4] + "_" + input_files[i][:-4] + "_thd_" + str(32) + "_" + str(count) + "_sleep_" + str(sz) + ".txt"
